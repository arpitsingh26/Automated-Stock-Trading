...
#parameters that will be taken as a part of testing data
priceopen=[]
priceclose=[]
pricelow=[]
pricehigh=[]
...
#taking the required parameters as a part of testing data from the given file
with open('fbtestgiven.csv', 'r') as csvfile:
	...
	for row in csvData:
		if i==0:
			...
			volumeclose.append(int(row[5])) #volumeclose for the interval is volume for the last day of the interval
		...
		if i==csize:
			...
			tempvolumelow=2147483647
			volumeopen.append(int(row[5])) #volumeopen for the interval is volume for the first day of the interval
			startdate.append(str(row[0]))
			arraysize=arraysize+1
			i=0

#writing data into testing file
with open('fbtestactual.csv', 'wb') as f:
    writer = csv.writer(f)
